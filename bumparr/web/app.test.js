  ...
  const body = el("body", {}, [
    ...
    el("div", { className: "shell" }, [
      el("nav", { id: "viewnav", className: "viewnav" },
         ["overview", "library", "composer", "station", "operations", "generation"].map(navLink)),
      el("main", { id: "main" }, [
        ...
        view("view-generation", [
          el("div", { id: "gen-banner" }), el("div", { id: "gen-privacy" }),
          el("div", { id: "gen-status" }), el("div", { id: "gen-message" }),
          el("select", { id: "gen-model", value: "" }),
          el("textarea", { id: "gen-brief", value: "" }),
          el("select", { id: "gen-duration", value: "" }),
          el("select", { id: "gen-resolution", value: "" }),
          el("select", { id: "gen-energy", value: "quiet" }),
          el("pre", { id: "gen-prompt" }), el("p", { id: "gen-estimate" }),
          el("button", { id: "gen-preflight" }), el("button", { id: "gen-submit", disabled: true }),
          el("div", { id: "gen-jobs" }), el("button", { id: "gen-jobs-prev" }), el("button", { id: "gen-jobs-next" }),
          el("select", { id: "gen-review-state", value: "pending" }), el("div", { id: "gen-review" }),
          el("button", { id: "gen-review-prev" }), el("button", { id: "gen-review-next" }),
        ]),
      ]),
    ]),
...
const { cardEl, pollJob, enableBumper, deleteBumper, stationEl, stationState,
        ...
        COMPOSER_PRESETS,
        REASON_TEXT, NO_PROVENANCE, renderStationConfig,
        renderGenerationJobs, renderReview, jobsNeedPoll, renderGenerationStatus,
        runPreflight, runCreate, invalidatePreflight, genRequest, fillModels,
        renderIfChanged, loadGeneration } = app;
...
function stubRoutes(over) {
  ...
  global.fetch = async (url, opts) => {
    ...
    if (u === "/api/generation") return jsonReply(o.generation || { enabled: false, budget: {} });
    if (u === "/api/generation/models") return jsonReply(o.models || { models: [] });
    if (u.startsWith("/api/generation/jobs")) return jsonReply(o.generationJobs || { jobs: [] });
    if (u.startsWith("/api/generation/outputs")) return jsonReply(o.generationOutputs || { outputs: [] });
    return jsonReply({});
  };
  return calls;
}
...
test("a direct Generation route hydrates shared status without reading Operations jobs", async () => {
  const calls = stubRoutes();
  await applyHash("#/generation");
  assert.equal(STATE.route, "generation");
  assert.equal(STATE.status.value.brand, "Bumparr");
  assert.match(textOf($("#header-profile")), /profile/);
  assert.ok(calls.some((call) => call.url === "/api/status"));
  assert.ok(calls.some((call) => call.url === "/api/generation"));
  assert.ok(!calls.some((call) => call.url === "/api/jobs"),
            "Generation must not start the independent Operations jobs read");
});

test("a terminal Generation queue does not schedule another five-second poll", async () => {
  ...
  try {
    stubRoutes({ generationJobs: { jobs: [{ id: "done", status: "completed" }] } });
    await applyHash("#/generation");
...
test("index.html keeps its landmarks, headings and hidden decoration", () => {
  ...
  assert.equal((html.match(/<h2\b/g) || []).length, 7,
               "six views plus the inspector, and no more");
...
test("queue exposes cancellation, reconciliation and paid regeneration", () => {
  renderGenerationJobs([{ id: "one", status: "queued" }, { id: "two", status: "submission_unknown" },
...
test("a preflight that resolves after leaving Generation cannot arm paid creation", async () => {
  generationForm();
  STATE.route = "generation";
  let resolve;
  global.fetch = async () => new Promise((done) => { resolve = done; });
  const pending = runPreflight();
  VIEWS.generation.exit();
  resolve({ ok: true, json: async () => preview });
  await pending;
  assert.equal($("#gen-submit").disabled, true);
});
